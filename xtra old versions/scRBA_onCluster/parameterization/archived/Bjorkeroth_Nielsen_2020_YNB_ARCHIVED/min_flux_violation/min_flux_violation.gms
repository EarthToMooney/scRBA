...
RiboCapacityMito.. 	v('RIBOSYN-ribomito') * %kribomito% * %nscale% =e= %mu% * sum(j$mito_translation(j), NAA(j) * v(j));
RiboCapacityNuc.. 	v('RIBOSYN-ribonuc') * %kribonuc% * %nscale% =e= %mu% * sum(j$nuc_translation(j), NAA(j) * v(j));
