*** Enzyme level calibration to the model constraints and projection ***
*       Author: Hoang Dinh
************************************************************************

$INLINECOM /*  */
$include "./enz_calibration_GAMS_settings.txt"
...
$include "%media_path%"
dvset(j) /*subset of index j to assign kapp variables to*/
$include "%noEnz_dvset_path%"
;
...
Variables
z, v(j), dv(j), venzSlack
;

dv.lo(j) = 0;
dv.up(j) = 1e5;
venzSlack.lo = 0; venzSlack.up = 1;
...
file ff /enz_calibration.modelStat.txt/;
put ff;
put rba.modelStat/;
putclose ff;

file ff2 /enz_calibration.flux.txt/;
put ff2;
loop(j,
	if ( (v.l(j) gt 1E-10),
...
file ff3 /enz_calibration.enzobj.txt/;
put ff3;
put v.l('%enzobj%'):0:11/;
putclose ff3;
